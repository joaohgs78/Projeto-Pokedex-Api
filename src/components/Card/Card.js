...
const Card = ({ pokemon, name, types, id, image }) => {
  ...
  const newName =
    capitalFirstLetter[0].toUpperCase() + capitalFirstLetter.substr(1);
  ...
  return (
    <S.ContainerCard style={{ backgroundColor }}>
      ...
      <S.DetailPokemon>
        <a onClick={() => goToPokemonDetailPage(navigate, name, types, id)}>
          Detalhes
        </a>
        {location.pathname === "/" && (
          <S.Capture>
